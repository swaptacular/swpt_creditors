...
Revision ID: 9ef66a0479b5
Revises: 8d8c816257ce
Create Date: 2020-01-13 15:38:21.417433
...
def upgrade():
    ...
    op.create_table('account_deletion_failure_signal',
    sa.Column('creditor_id', sa.BigInteger(), nullable=False),
    sa.Column('signal_id', sa.BigInteger(), autoincrement=True, nullable=False),
    sa.Column('debtor_id', sa.BigInteger(), nullable=False),
    sa.Column('deleted_at_ts', sa.TIMESTAMP(timezone=True), nullable=False),
    sa.Column('deletion_failed_at_ts', sa.TIMESTAMP(timezone=True), nullable=False),
    ...
    op.create_table('created_account',
    sa.Column('creditor_id', sa.BigInteger(), nullable=False),
    sa.Column('debtor_id', sa.BigInteger(), nullable=False),
    sa.Column('created_at_ts', sa.TIMESTAMP(timezone=True), nullable=False),
    sa.PrimaryKeyConstraint('creditor_id', 'debtor_id'),
    comment="Represents a created account from users' perspecive. Note that the account may still have no corresponding `Account` record."
    )
    ...
    op.create_table('account_deletion_request',
    sa.Column('creditor_id', sa.BigInteger(), nullable=False),
    sa.Column('debtor_id', sa.BigInteger(), nullable=False),
    sa.Column('sent_at_ts', sa.TIMESTAMP(timezone=True), nullable=False, comment='The moment at which the last `mark_account_for_deletion` signal was sent.'),
    sa.Column('ignore_after_ts', sa.TIMESTAMP(timezone=True), nullable=False, comment='The `ignore_after_ts` parameter, sent with the last `mark_account_for_deletion` signal. Must never decrease.'),
    sa.Column('negligible_amount', sa.BigInteger(), nullable=False, comment='The `negligible_amount` parameter, sent with the last `mark_account_for_deletion` signal.'),
    sa.Column('is_canceled', sa.BOOLEAN(), nullable=False, comment='Whether the request was canceled or not. A `True` also implies that there is no corresponding `DeletedAccount` record.'),
    sa.ForeignKeyConstraint(['creditor_id', 'debtor_id'], ['created_account.creditor_id', 'created_account.debtor_id'], ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('creditor_id', 'debtor_id'),
    ...
    sa.PrimaryKeyConstraint('creditor_id', 'debtor_id', 'transfer_seqnum', 'new_account_principal'),
    ...
    sa.Column('debtor_id', sa.BigInteger(), nullable=False),
    sa.Column('deleted_at_ts', sa.TIMESTAMP(timezone=True), nullable=False),
    sa.ForeignKeyConstraint(['creditor_id', 'debtor_id'], ['account_deletion_request.creditor_id', 'account_deletion_request.debtor_id'], ondelete='CASCADE'),
    sa.ForeignKeyConstraint(['creditor_id', 'debtor_id'], ['created_account.creditor_id', 'created_account.debtor_id'], ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('creditor_id', 'debtor_id'),
    comment="Represents a deleted account from users' perspecive. Note that the account may still have a corresponding `Account` record."
    )
...
def downgrade():
    ...
    op.drop_table('account_deletion_failure_signal')
    # ### end Alembic commands ###
