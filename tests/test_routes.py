...
def test_create_creditor(client):
    ...
    r = client.post(
        "/creditors/4294967296/reserve",
        headers={"X-Swpt-User-Id": "creditors:2"},
        json={},
    )
    assert r.status_code == 403
    ...
    entries = _get_all_pages(
        client,
        "/creditors/4294967296/log",
        page_type="LogEntriesPage",
        streaming=True,
    )
    ...
    r = client.post(
        "/creditors/4294967297/deactivate",
...
def test_change_pin(client, creditor):
    r = client.get(
        "/creditors/4294967296/wallet", headers={"X-Swpt-Require-Pin": "true"}
    )
    assert r.status_code == 200
    data = r.get_json()
    ...
    r = client.get(
        "/creditors/4294967296/wallet", headers={"X-Swpt-Require-Pin": "false"}
    )
    assert r.status_code == 200
    data = r.get_json()
    assert data["requirePin"] is False
...
def test_redirect_to_wallet(client, creditor):
    ...
    r = client.get(
        "/creditors/.wallet",
        headers={"X-Swpt-User-Id": "creditors:4294967296"},
    )
    assert r.status_code == 303
    assert (
        r.headers["Location"] == "http://localhost/creditors/4294967296/wallet"
    )

    r = client.get(
        "/creditors/.wallet",
        headers={"X-Swpt-User-Id": "creditors:18446744073709551615"},
    )
    assert r.status_code == 303
    assert (
        r.headers["Location"]
        == "http://localhost/creditors/18446744073709551615/wallet"
    )
...
def test_accounts_list_page(client, account):
    ...
    items = _get_all_pages(
        ...
        page_type="ObjectReferencesPage",
    )
    ...
    assert (
        u64_to_i64(9223372036854775808)
        < u64_to_i64(9223372036854775809)
        < u64_to_i64(1)
    )
    ...
    assert all(
        [datetime.fromisoformat(e["addedAt"]) not in e for e in entries]
    )
...
def test_transfers_list_page(client, account, creditor):
    ...
    client.post(
        "/creditors/4294967296/transfers/",
        json={**request_data, "transferUuid": uuid_pattern.format("000")},
    )
    items = _get_all_pages(
        client,
        "/creditors/4294967296/transfers/",
    ...
    client.post(
        "/creditors/4294967296/transfers/",
        json={**request_data, "transferUuid": uuid_pattern.format("002")},
    )
    p.process_pending_log_entries(4294967296)
    client.post(
        "/creditors/4294967296/transfers/",
        json={**request_data, "transferUuid": uuid_pattern.format("001")},
    )
    p.process_pending_log_entries(4294967296)
    items = _get_all_pages(
        client,
    ...
    assert [
        (e["objectType"], e["object"]["uri"], e.get("objectUpdateId"))
        for e in entries
    ] == [
...
def test_debtor_lookup(client, account):
    r = client.post(
        "/creditors/4294967296/debtor-lookup",
    ...
    r = client.post(
        "/creditors/4294967296/debtor-lookup",
        json={"type": "DebtorIdentity", "uri": "swpt:1"},
    )
    assert r.status_code == 303
    assert (
        r.headers["Location"]
        == "http://localhost/creditors/4294967296/accounts/1/"
    )

    r = client.post(
        "/creditors/4294967296/debtor-lookup",
        json={"type": "DebtorIdentity", "uri": "swpt:1111"},
    )
    assert r.status_code == 204
    assert r.data == b""
...
def test_create_account(client, creditor):
    ...
    r = client.post(
        "/creditors/4294967296/accounts/",
        json={"type": "DebtorIdentity", "uri": "xxx:1"},
    )
    assert r.status_code == 422
    data = r.get_json()
    assert data["errors"]["json"]["uri"] == ["The URI can not be recognized."]
...
def test_delete_account(client, account):
    ...
    latestUpdateId = p.get_account_config(
        4294967296, 1
    ).config_latest_update_id
    r = client.patch(
        "/creditors/4294967296/accounts/1/config",
        json={
    ...
    assert [
        ...
        (
            "AccountDisplay",
            "/creditors/4294967296/accounts/1/display",
        ...
        (
            "Account",
            "/creditors/4294967296/accounts/1/",
            object_update_id,
            True,
        ),
        (
        ...
        (
            "AccountInfo",
            "/creditors/4294967296/accounts/1/info",
            object_update_id,
            True,
        ),
        (
        ...
        (
            "AccountExchange",
            "/creditors/4294967296/accounts/1/exchange",
            object_update_id,
            True,
        ),
        (
            "AccountKnowledge",
            "/creditors/4294967296/accounts/1/knowledge",
            object_update_id,
            True,
        ),
    ]
...
def test_account_config(client, account):
    ...
    r = client.patch(
        "/creditors/4294967296/accounts/1111/config", json=request_data
    )
    assert r.status_code == 404
...
def test_account_display(client, account):
    ...
    r = client.patch(
        "/creditors/4294967296/accounts/1111/display", json=request_data
    )
    assert r.status_code == 404

    r = client.patch(
        "/creditors/4294967296/accounts/1/display", json=request_data
    )
    assert r.status_code == 200
    r = client.patch(
        "/creditors/4294967296/accounts/1/display", json=request_data
    ...
    r = client.patch(
        "/creditors/4294967296/accounts/11/display",
        json={**orig_request_data, "latestUpdateId": latestUpdateId_11 + 1},
    )
    assert r.status_code == 422
    data = r.get_json()
    ...
    assert [
        (e["objectType"], e["object"]["uri"], e["objectUpdateId"])
        for e in entries
    ] == [
    ...
    assert all(
        [
            entries[i]["entryId"] - entries[i - 1]["entryId"] == 1
...
def test_account_exchange(client, account):
    ...
    r = client.post("/creditors/4294967296/accounts/", json={"uri": "swpt:11"})
    assert r.status_code == 201
    config_latest_update_id = p.get_account_config(
        4294967296, 11
    ).config_latest_update_id
    p.process_pending_log_entries(4294967296)
    r = client.patch(
        "/creditors/4294967296/accounts/11/config",
        json={
            "scheduledForDeletion": True,
            "negligibleAmount": m.DEFAULT_NEGLIGIBLE_AMOUNT,
            "allowUnsafeDeletion": True,
    ...
    assert r.status_code == 422
    data = r.get_json()
    assert data["errors"]["json"]["policy"] == [
    ...
    request_data["peg"] = {
        "exchangeRate": 1.5,
        "account": {"uri": "/creditors/4294967296/accounts/1111/"},
    }
    r = client.patch(
        "/creditors/4294967296/accounts/1/exchange", json=request_data
    )
    assert r.status_code == 422
    data = r.get_json()
    assert data["errors"]["json"]["peg"]["account"]["uri"] == [
    ...
    request_data["peg"]["account"][
        "uri"
    ] = "/creditors/4294967296/accounts/11/"
    r = client.patch(
        "/creditors/4294967296/accounts/1/exchange", json=request_data
    )
    assert r.status_code == 200
...
def test_account_knowledge(client, account):
    ...
    r = client.patch(
        "/creditors/4294967296/accounts/1111/knowledge", json=request_data
    )
    assert r.status_code == 404

    r = client.patch(
        "/creditors/4294967296/accounts/1/knowledge", json=request_data
    )
    assert r.status_code == 200
    r = client.patch(
        "/creditors/4294967296/accounts/1/knowledge", json=request_data
    ...
    assert datetime.fromisoformat(data["interestRateChangedAt"]) == datetime(
...
def test_get_account_ledger(client, account):
    ...
    assert data["entries"] == {
        ...
        "type": "PaginatedList",
        "first": (
            f"/creditors/4294967296/accounts/1/entries?prev={next_entry_id}"
        ),
    }
    assert data["account"] == {"uri": "/creditors/4294967296/accounts/1/"}
...
def test_ledger_entries_list(ledger_entries, client, current_ts):
    ...
    items = _get_all_pages(
        client,
        "/creditors/4294967296/accounts/1/entries?prev=1000000000000000",
    ...
    assert items == [
        {
            ...
            "principal": 350,
            "transfer": {
                "uri": "/creditors/4294967296/accounts/1/transfers/0-2"
            },
        },
        {
            "type": "LedgerEntry",
        ...
        },
        {
            ...
            "principal": 100,
            "transfer": {
                "uri": "/creditors/4294967296/accounts/1/transfers/0-1"
            },
        },
    ]
    ...
    items = _get_all_pages(
        client,
        f"/creditors/4294967296/accounts/1/entries?prev={first_entry_id}",
        page_type="LedgerEntriesPage",
    )
    assert len(items) == 0
    ...
    items = _get_all_pages(
        client,
        "/creditors/4294967296/accounts/1/entries"
        f"?prev=1000000000000000&stop={first_entry_id}",
        page_type="LedgerEntriesPage",
    )
    assert len(items) == 2
    ...
    items = _get_all_pages(
        client,
        "/creditors/4294967296/accounts/1/entries"
        f"?prev={first_entry_id + 2}&stop={first_entry_id}",
        page_type="LedgerEntriesPage",
    )
    assert len(items) == 1
    ...
    items = _get_all_pages(
        client,
        "/creditors/4294967296/accounts/1/entries"
        f"?prev={first_entry_id + 1}&stop=1000000000000000",
        page_type="LedgerEntriesPage",
    )
    assert len(items) == 0
    ...
    items = _get_all_pages(
        client,
        "/creditors/4294967296/accounts/1/entries"
        f"?prev={first_entry_id + 1}&stop={first_entry_id + 1}",
        page_type="LedgerEntriesPage",
    )
    assert len(items) == 0
    ...
    assert [
        ...
        (
            ...
            ledger_latest_update_id - 1,
            {
                ...
                "firstPage": (
                    "/creditors/4294967296/accounts/1/"
                    f"entries?prev={first_entry_id + 1}"
                ),
            },
        ),
        (
...
def test_get_committed_transfer(client, account, current_ts):
    ...
    r = client.get(
        "/creditors/4294967296/accounts/1/transfers/"
...
def test_create_transfer(client, account):
    ...
    assert (
        r.headers["location"]
        == "http://localhost/creditors/4294967296/transfers/"
        "123e4567-e89b-12d3-a456-426655440000"
    )
    p.process_pending_log_entries(4294967296)

    r = client.get(
        "/creditors/4294967296/transfers/123e4567-e89b-12d3-a456-426655440000"
    )
    assert r.status_code == 200
    data = r.get_json()
    ...
    assert (
        data["transfersList"]["uri"] == "/creditors/4294967296/transfers-list"
    )
    ...
    r = client.post(
        "/creditors/4294967296/transfers/",
        json={**request_data, "amount": 999},
    )
    assert r.status_code == 409

    r = client.post(
        "/creditors/4294967296/transfers/",
        json={**request_data, "recipient": {"uri": "INVALID"}},
    )
    assert r.status_code == 422
    assert r.get_json()["errors"]["json"]["recipient"]["uri"] == [
        "The URI can not be recognized."
    ]

    r = client.post(
        "/creditors/4294967296/transfers/123e4567-e89b-12d3-a456-426655440001",
        json={},
    )
    assert r.status_code == 404
    ...
    r = client.post(
        "/creditors/4294967298/transfers/",
        headers={"X-Swpt-Require-Pin": "true"},
        json=request_data,
    )
    assert r.status_code == 404
...
def test_unauthorized_creditor_id(creditor, client):
    ...
    r = client.get(
        "/creditors/4294967296/",
        headers={"X-Swpt-User-Id": "creditors:4294967298"},
    )
    assert r.status_code == 403

    r = client.get(
        "/creditors/18446744073709551615/",
        headers={"X-Swpt-User-Id": "creditors:18446744073709551615"},
    )
    assert r.status_code == 404
    ...
    with pytest.raises(ValueError):
        r = client.get(
            "/creditors/18446744073709551615/",
            headers={"X-Swpt-User-Id": "creditors:18446744073709551616"},
        )
...
def test_pin_afa_reset(app, client, creditor, account, reset_days):
    ...
    r = client.post(
        "/creditors/4294967296/transfers/", headers=headers, json=incorrect_pin
    )
    assert r.status_code == 403
