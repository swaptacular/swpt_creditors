...
def has_account(creditor_id: int, debtor_id: int) -> bool:
    account_query = Account.query.filter_by(
...
def delete_account(creditor_id: int, debtor_id: int) -> None:
    ...
    if not (
        ...
        or data.allow_unsafe_deletion
        or display.debtor_name is None
    ):
...
def get_account_config(
    creditor_id: int, debtor_id: int, lock=False
) -> Optional[AccountData]:
    query = AccountData.query.filter_by(
        creditor_id=creditor_id, debtor_id=debtor_id
    ...
    *,
    ...
    db.session.add(
        PendingLogEntry(
            ...
            object_uri=paths.account_config(
                creditorId=creditor_id, debtorId=debtor_id
            ),
...
def get_account_display(
    creditor_id: int, debtor_id: int, lock=False
) -> Optional[AccountDisplay]:
    query = AccountDisplay.query.filter_by(
        creditor_id=creditor_id, debtor_id=debtor_id
    )
    if lock:
        query = query.with_for_update()
    ...
    *,
    ...
    db.session.add(
        PendingLogEntry(
            ...
            object_uri=paths.account_display(
                creditorId=creditor_id, debtorId=debtor_id
            ),
            object_update_id=latest_update_id,
        )
...
@atomic
def get_account_knowledge(
    creditor_id: int, debtor_id: int, lock=False
) -> Optional[AccountKnowledge]:
    query = AccountKnowledge.query.filter_by(
        creditor_id=creditor_id, debtor_id=debtor_id
    )
    if lock:
        query = query.with_for_update()
...
def update_account_knowledge(
    creditor_id: int, debtor_id: int, *, latest_update_id: int, data: dict
) -> AccountKnowledge:
    current_ts = datetime.now(tz=timezone.utc)
    ...
    try:
        perform_update = allow_update(
            knowledge, "latest_update_id", latest_update_id, {"data": data}
        )
    except errors.AlreadyUpToDate:
        return knowledge
    ...
    db.session.add(
        PendingLogEntry(
            ...
            object_uri=paths.account_knowledge(
                creditorId=creditor_id, debtorId=debtor_id
            ),
            object_update_id=latest_update_id,
        )
...
@atomic
def get_account_exchange(
    creditor_id: int, debtor_id: int, lock=False
) -> Optional[AccountExchange]:
    query = AccountExchange.query.filter_by(
        creditor_id=creditor_id, debtor_id=debtor_id
    )
    if lock:
        query = query.with_for_update()
...
def update_account_exchange(
    ...
    *,
    ...
    if peg_debtor_id is not None and not has_account(
    ...
    db.session.add(
        PendingLogEntry(
            ...
            object_uri=paths.account_exchange(
                creditorId=creditor_id, debtorId=debtor_id
            ),
            object_update_id=latest_update_id,
        )
...
def _insert_account(
    creditor: Creditor, debtor_id: int, current_ts: datetime
) -> Account:
    ...
    data = AccountData(
        ...
        ledger_last_entry_id=creditor.largest_historic_ledger_entry_id
        + 1,  # a gap
    )
    account = Account(
        creditor_id=creditor_id,
...
def _log_account_deletion(
    ...
    for object_type, object_uri in [
        (
            types.account,
            paths.account(creditorId=creditor_id, debtorId=debtor_id),
        ),
        (
            types.account_config,
            paths.account_config(creditorId=creditor_id, debtorId=debtor_id),
        ),
        (
            types.account_info,
            paths.account_info(creditorId=creditor_id, debtorId=debtor_id),
        ),
        (
            types.account_ledger,
            paths.account_ledger(creditorId=creditor_id, debtorId=debtor_id),
        ),
        (
            types.account_display,
            paths.account_display(creditorId=creditor_id, debtorId=debtor_id),
        ),
        (
            types.account_exchange,
            paths.account_exchange(creditorId=creditor_id, debtorId=debtor_id),
        ),
        (
            types.account_knowledge,
            paths.account_knowledge(
                creditorId=creditor_id, debtorId=debtor_id
            ),
        ),
    ]:
...
def _insert_info_update_pending_log_entry(
    data: AccountData, current_ts: datetime
) -> None:
    paths, types = get_paths_and_types()
